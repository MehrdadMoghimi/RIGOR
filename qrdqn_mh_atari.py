...
def make_env(env_id, seed, num_envs, capture_video, run_name, **env_kwargs):
    """Create efficient ALE vector environment for Atari games."""
    time_aware = env_kwargs.get('time_aware', False)
    
    # Extract game name from env_id (handles both "ALE/Pong-v5" and "BreakoutNoFrameskip-v4" formats)
    #if env_id.startswith("ALE/"):
    #    # Format: "ALE/Pong-v5" -> "pong"
    #    game_name = env_id.split("/")[1].split("-")[0].lower()
    #else:
    #    # Format: "BreakoutNoFrameskip-v4" -> "breakout"
    #    game_name = env_id.replace("NoFrameskip", "").split("-")[0].lower()
    
    # Use efficient ALE vector environment directly
    envs = AtariVectorEnv(
        game=env_id,
        num_envs=num_envs,
        # Preprocessing parameters (matches standard Atari preprocessing)
        frameskip=4,
        grayscale=True,
        stack_num=4,
        img_height=84,
        ...
        # Performance options
        batch_size=0,  # 0 means process all envs at once
        autoreset_mode=gym.vector.AutoresetMode.NEXT_STEP,
        num_threads=0,  # Auto-detect optimal thread count
        thread_affinity_offset=-1,  # No thread affinity by default
    )
    
    # Note: AtariVectorEnv is already a vector env, can't use single-env wrappers
    # Episode statistics tracking will be handled manually in the training loop
    
    # Handle video recording for first environment only
    if capture_video:
        print(f"Warning: Video recording not yet implemented for AtariVectorEnv")
...
class QNetwork(nn.Module):
    def __init__(self, n_obs, n_act, n_gammas, n_quantiles, device=None, time_aware=False):
        super().__init__()
        self.n_act = n_act
        ...
        if time_aware:
            self.time_layer = nn.Sequential(
                nn.Linear(1, 64, device=device),
                nn.ReLU(),
            )
            linear_input_size += 64

        self.fc = nn.Sequential(
            nn.Linear(linear_input_size, 512, device=device),
            nn.ReLU(),
            nn.Linear(512, n_act * n_gammas * n_quantiles, device=device),
        )
    ...
    def forward(self, x, time_steps=None):
        ...
        if self.time_aware:
            if time_steps is None:
                raise ValueError("time_steps must be provided when time_aware is True")
            t = time_steps.float().view(-1, 1)/1000.0  # normalize time steps
            t = self.time_layer(t)
            x = torch.cat([x, t], dim=1)
            
        logits = self.fc(x)
        return logits.view(len(x), self.n_act, self.n_gammas, self.n_quantiles)
    
    def get_action(self, x, action=None, time_steps=None, **kwargs):
        quantiles = self.forward(x, time_steps=time_steps) # (batch_size, n_act, n_gammas, n_quantiles)
...
if __name__ == "__main__":
    ...
    envs = make_env(args.env_id, args.seed, args.num_envs, args.capture_video, run_name, **env_kwargs)
    print(f"Using efficient ALE vector environment with {args.num_envs} parallel environments")
    ...
    q_network = QNetwork(n_obs=n_obs, n_act=n_act, n_gammas=args.n_gammas, n_quantiles=args.n_quantiles, device=device, time_aware=args.time_aware)
    optimizer = optim.Adam(q_network.parameters(), lr=args.learning_rate, eps=0.01 / args.batch_size)
    target_network = QNetwork(n_obs=n_obs, n_act=n_act, n_gammas=args.n_gammas, n_quantiles=args.n_quantiles, device=device, time_aware=args.time_aware)
    ...
    for global_step in pbar:
        ...
        if python_rng.random() < epsilon:
            ...
            actions, _ = q_network.get_action(torch.tensor(obs, device=device), time_steps=torch.tensor(episode_lengths, device=device), **get_action_kwargs)
        ...
        # Add episode statistics to infos
        if "episode" not in infos:
            infos["episode"] = {}
        infos["episode"]["r"] = episode_returns.copy()
        infos["episode"]["l"] = episode_lengths.copy()
        ...
        if global_step > args.learning_starts:
            if global_step % args.train_frequency == 0:
                ...
                with torch.no_grad():
                    _, next_quantiles = target_network.get_action(data.next_observations, time_steps=data.episode_lengths.squeeze(),**get_action_kwargs) # (batch_size, n_gammas, n_quantiles)
                    target_quantiles = data.rewards.unsqueeze(-1) + gammas[None, :, None] * next_quantiles * (1 - data.dones.unsqueeze(-1))

                _, old_quantiles = q_network.get_action(data.observations, data.actions.flatten(), time_steps=data.episode_lengths.squeeze() - 1) # (batch_size, n_gammas, n_quantiles)
