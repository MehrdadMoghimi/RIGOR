# https://github.com/google-research/google-research/blob/master/hyperbolic_discount/agent_utils.py
# Credit: Google Research Authors.
import math
...
def compute_eval_gamma_interval(gamma_max, hyp_exponent, number_of_gammas):
  ...
  # gamma being used.
    ...
    print('Running baseline model with single head.')
...
def integrate_q_values(q_values, integral_estimate, eval_gammas, number_of_gammas, gammas):
  ...
  if integral_estimate == 'lower':
    ...
    for g, w in zip(gammas, weights):
      print(' V_{gamma=%0.5f} weight=%.3f', g, w)
    print('self.eval_gammas: %s', eval_gammas)
    print('self.gammas: %s', gammas)
