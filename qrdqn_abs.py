...
class QNetwork(nn.Module):
    ...
    def get_action(self, x, action=None, **kwargs):
        quantiles = self.forward(x) # (batch_size, n_act, n_quantiles)

        if action is None:
            reward_normalizer = kwargs.get('reward_normalizer', 1.0)
            stock_batch = reward_normalizer * x[:, -1].reshape(-1, 1, 1)
            sp_quantiles = quantiles + stock_batch
            sp_quantiles = -torch.abs(sp_quantiles)
            q_values = sp_quantiles.mean(2)
